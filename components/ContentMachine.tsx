...
const CONTENT_SYSTEM_PROMPT =
  "You are an expert social media content creator for local businesses. " +
...
export function ContentMachine({ locked }: { locked: boolean }) {
  ...
  const [refinedPosts, setRefinedPosts] = useState<string | null>(null);
    ...
    <div className="flex-1 p-7 max-w-5xl">
      ...
      {content && (
        <div id="content-results">
          ...
          {/* Refined output */}
          {refinedPosts && (
            <div className="mt-4 bg-[#111214] border border-[#00e5a0]/20 rounded-2xl p-5">
              <p className="text-xs font-semibold text-[#00e5a0] uppercase tracking-widest mb-3">✨ Refined Version</p>
              <p className="text-sm text-[#d1d5db] leading-relaxed whitespace-pre-wrap">{refinedPosts}</p>
            </div>
          )}
          ...
          <div className="mt-4">
            <RefineChat
              originalResult={content.posts.map((p, i) =>
                `Post ${i + 1} (${p.platform}):\n${p.content}\nHashtags: ${p.hashtags.join(" ")}`
              ).join("\n\n")}
              agentType="content"
              systemPrompt={CONTENT_SYSTEM_PROMPT}
              quickActions={["More hashtags", "Make it funnier", "More professional", "Add call to action", "Shorter posts"]}
              onRefined={(text) => setRefinedPosts(text)}
            />
          </div>
        </div>
      )}
