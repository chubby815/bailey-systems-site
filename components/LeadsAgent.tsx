...
function LeadCard({ lead, index }: { lead: Lead; index: number }) {
  ...
  function handleSave() {
    ...
    const alreadySaved = existing.some(
      (s) => s.businessName === lead.businessName && s.email === lead.email
    );
    if (!alreadySaved) {
      existing.push(lead);
  ...
  return (
    <div className="bg-[#111214] border border-white/[0.07] rounded-2xl p-5 hover:border-white/[0.14] transition-all">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <h3 className="font-bold text-[#f0f0f0] text-sm">{lead.businessName}</h3>
            <span className="text-[10px] text-[#6b7280] bg-white/[0.03] border border-white/[0.06] px-2 py-0.5 rounded-full">
              {lead.industry}
            </span>
          </div>
          <p className="text-xs text-[#6b7280]">
            {lead.ownerName} · {lead.location} · {lead.estimatedSize}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <button
          onClick={() => copyText(lead.phone, `phone-${index}`)}
          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all ${
            copied === `phone-${index}`
              ? "text-[#00e5a0] border-[#00e5a0]/30 bg-[#00e5a0]/5"
              : "text-[#9ca3af] border-white/[0.07] bg-white/[0.02] hover:text-white hover:border-white/20"
          }`}
        >
          📞 {copied === `phone-${index}` ? "Copied!" : lead.phone}
        </button>
        <button
          onClick={() => copyText(lead.email, `email-${index}`)}
          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all ${
            copied === `email-${index}`
              ? "text-[#00e5a0] border-[#00e5a0]/30 bg-[#00e5a0]/5"
              : "text-[#9ca3af] border-white/[0.07] bg-white/[0.02] hover:text-white hover:border-white/20"
        ...
        >
          ✉ {copied === `email-${index}` ? "Copied!" : lead.email}
        </button>
      </div>

      {/* Score reason */}
      <p className="text-[11px] text-[#4b5563] mb-3 leading-relaxed italic">
        {lead.scoreReason}
      </p>
      ...
      <div className="border border-white/[0.06] rounded-xl overflow-hidden">
        ...
        {expanded && (
          <div className="px-4 pb-4 border-t border-white/[0.06]">
            ...
            <button
              onClick={() => copyText(lead.outreachMessage, `msg-${index}`)}
            ...
            >
              {copied === `msg-${index}` ? "✓ Copied!" : "Copy Message"}
            </button>
          </div>
        )}
      </div>
    ...
    <div className="flex-1 p-7">
      <div className="max-w-lg">
        ...
        <p className="text-gray-400 text-sm mb-8">
          Find qualified leads in your area and generate personalized outreach messages.
        </p>
        ...
        <div className="bg-[#111214] border border-white/[0.07] rounded-2xl p-10 text-center">
          ...
          <p className="text-gray-500 text-sm max-w-sm mx-auto mb-6 leading-relaxed">
            Upgrade to Growth or Pro to unlock the Lead Hunter agent. Find qualified local
            business leads and generate personalized outreach in one click.
          </p>
          <Link
            href="/pricing"
  ...
  return (
    <div className="flex-1 p-7 max-w-5xl">
      ...
      <p className="text-gray-400 text-sm mb-7">
        Enter your target market below and our AI will find qualified local business leads with personalized outreach messages.
      </p>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select value={form.businessSize} onChange={(e) => set("businessSize", e.target.value)}
              className={`${INPUT_CLS} appearance-none cursor-pointer`}>
        ...
        <>
          ...
          <div className="flex items-center gap-6 mb-5 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-[#6b7280]">leads found</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-[#6b7280]">avg score</span>
            </div>
            <div className="w-px h-6 bg-white/[0.07]" />
            <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-[#4b5563] mt-6 text-center">
            Leads are AI-generated for prospecting purposes. Always verify contact info before reaching out.
          </p>
        </>
      )}
      ...
      {searched && leads.length === 0 && !loading && (
        <div className="bg-[#111214] border border-white/[0.07] rounded-2xl p-10 text-center">
          ...
          <p className="text-gray-400 text-sm">No leads returned. Try adjusting your search filters.</p>
        </div>
      )}
    </div>
  );
