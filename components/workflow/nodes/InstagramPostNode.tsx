...
type InstagramPostData = { label: string; caption: string; imageUrl: string }
type InstagramPostNodeType = Node<InstagramPostData, 'instagramPost'>

export function InstagramPostNode({ id, data, selected }: NodeProps<InstagramPostNodeType>) {
  const { updateNodeData } = useReactFlow()
  const update = useCallback(
    (patch: Partial<InstagramPostData>) => updateNodeData(id, patch),
    [id, updateNodeData],
  )
  return (
    <div style={{
      ...
      <div style={{ fontSize: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        <div>
          <label style={{ color: '#6b7280', display: 'block', marginBottom: '0.2rem' }}>Image URL</label>
          <input
            value={data.imageUrl || ''}
            onChange={e => update({ imageUrl: e.target.value })}
            className="nodrag nopan"
            placeholder="https://... or {{imageVar}}"
            style={{
        ...
        <div>
          <label style={{ color: '#6b7280', display: 'block', marginBottom: '0.2rem' }}>Caption</label>
          <textarea
            value={data.caption || ''}
        ...
        <span style={{ color: '#4b5563', fontSize: '0.65rem' }}>Posts via connected Instagram Business Account</span>
      </div>
