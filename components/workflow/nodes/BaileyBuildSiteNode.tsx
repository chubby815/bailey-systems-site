...
type BaileyBuildSiteData = { label: string; businessName: string; industry: string; tone: string }
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {[
          { key: 'businessName', label: 'Business Name', placeholder: 'Chubbys Fitness' },
          { key: 'industry', label: 'Industry', placeholder: 'Gym' },
          { key: 'tone', label: 'Tone', placeholder: 'Luxury' },
        ].map(({ key, label, placeholder }) => (
          <div key={key}>
            <label style={{ color: '#6b7280', display: 'block', marginBottom: '0.15rem' }}>{label}</label>
            <input
              value={(data[key as keyof BaileyBuildSiteData] as string) || ''}
              onChange={e => update({ [key]: e.target.value } as Partial<BaileyBuildSiteData>)}
