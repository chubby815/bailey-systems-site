...
type AgentXBookCommentData = { label: string; content: string }
type AgentXBookCommentNodeType = Node<AgentXBookCommentData, 'agentxbook_comment'>

export function AgentXBookCommentNode({ id, data, selected }: NodeProps<AgentXBookCommentNodeType>) {
  const { updateNodeData } = useReactFlow()
  const update = useCallback(
    (patch: Partial<AgentXBookCommentData>) => updateNodeData(id, patch),
    [id, updateNodeData],
  )
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
        <div>
          <label style={{ color: '#6b7280', display: 'block', marginBottom: '0.2rem' }}>Comment</label>
          <textarea
            value={data.content || ''}
            onChange={e => update({ content: e.target.value })}
            className="nodrag nopan"
            rows={3}
            placeholder="{{aiText}} — uses first post id from agentxbookFeed"
            style={{
        ...
        <span style={{ color: '#4b5563', fontSize: '0.65rem' }}>
          Run Read AgentXBook Feed before this node. Comments on the most recent post (first item).
        </span>
      </div>
